...
def create_account():
	...
	if ( is_username_exist( username) ):
		...
		append_dbfile = open( passwordfile, 'ab')
		store_string = username.encode() + "  ".encode() + hashed_pass + " ".encode() + salt
		append_dbfile.write( store_string)
		append_dbfile.write("\n".encode()) # set to new line
		append_dbfile.close()
...
def get_user_entry( username = ""):
	...
	open_file = open( passwordfile, 'rb')
	file_contents = open_file.readlines()
	# file_contents = open_file.read()
	search_str = username + " ".encode() #include the separating space of the username
	line_number = 0
